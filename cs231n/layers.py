...
def batchnorm_backward(dout, cache):
    ...
    dvar = np.sum(dx_hat * x_mu * -0.5 * inv_std**3, axis=0)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    out = gamma * x_hat + beta                       # broadcast over N
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx_padded[n, :, h_start:h_end, w_start:w_end] += w[f] * dout[
                        n, f, i, j
                    ]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_group = x.reshape(N, G, C // G, H, W)           # (N, G, C//G, H, W)
    x_group_flat = x_group.reshape(N, G, -1)          # (N, G, group_size)

    mean = np.mean(x_group_flat, axis=2, keepdims=True)  # (N, G, 1)
    var = np.var(x_group_flat, axis=2, keepdims=True)    # (N, G, 1)

    x_groupnorm_flat = (x_group_flat - mean) / np.sqrt(var + eps)
    x_groupnorm = x_groupnorm_flat.reshape(N, G, C // G, H, W)
    x_hat = x_groupnorm.reshape(N, C, H, W)

    out = gamma * x_hat + beta

    cache = (G, x_hat, mean, var, gamma, beta, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, x_hat, mean, var, gamma, beta, eps = cache
    N, C, H, W = x_hat.shape
    group_size = (C // G) * H * W
    ...
    x_hat_group = x_hat.reshape(N, G, C // G, H, W).reshape(N, G, -1)

    inv_std = 1.0 / np.sqrt(var + eps)  # (N, G, 1)
    D = dx_hat_group.shape[2]

    dx_group = (1.0 / D) * inv_std * (
        D * dx_hat_group
