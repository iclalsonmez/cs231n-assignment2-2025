...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                ...
                out, bn_cache = batchnorm_forward(
                    out, gammai, betai, self.bn_params[i - 1]
                )
            elif self.normalization == "layernorm":
                gammai = self.params["gamma%d" % i]
                betai = self.params["beta%d" % i]
                out, bn_cache = layernorm_forward(
                    out, gammai, betai, self.bn_params[i - 1]
                )
        ...
        # Last affine layer (no ReLU / dropout / norm)
        ...
        grads["W%d" % self.num_layers] = dW_last + self.reg * self.params[
            "W%d" % self.num_layers
        ]
